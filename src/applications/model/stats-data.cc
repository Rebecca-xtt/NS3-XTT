...
/////phy packet tx xtt resize
 // phyTxPkts = (phyTxPkts * (iteration-1) + s.phyTxPkts) / iteration;
...
//phy statistic xtt revise
  //s.phyTxPkts = m_scalarData.phyTxPkts;
...
//xtt revise
    //out << "Phy Tx packets:," << s.phyTxPkts << std::endl;
...
//UDP statistic drop a packet due to buffer overflow
  Config::Connect ("/NodeList/*/$ns3::UdpL4Protocol/SocketList/*/Drop", MakeCallback (&StatsFlows::UdpPacketDrop, this));
// IPV4
  Config::Connect ("/NodeList/*/$ns3::Ipv4L3Protocol/Tx", MakeCallback (&StatsFlows::Ipv4PacketSent, this));
  Config::Connect ("/NodeList/*/$ns3::Ipv4L3Protocol/Rx", MakeCallback (&StatsFlows::Ipv4PacketReceive, this));
 // Config::Connect ("/NodeList/*/$ns3::Ipv4L3Protocol/Drop", MakeCallback (&StatsFlows::Ipv4PacketDrop, this));
...
//PHY tx rxok  statistic
  Config::Connect ("/NodeList/*/DeviceList/*/Phy/State/Tx", MakeCallback (&StatsFlows::PhyPacketSent, this));
  Config::Connect ("/NodeList/*/DeviceList/*/Phy/State/RxOk", MakeCallback (&StatsFlows::PhyPacketReceive, this));
//MAC  statistic
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/MacTx", MakeCallback (&StatsFlows::MacPacketSent, this));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::WifiNetDevice/Mac/MacRx", MakeCallback (&StatsFlows::MacPacketReceive, this));


  //NodeList/[i]/DeviceList/[i]/$ns3::WifiNetDevice/Phy/$ns3::YansWifiPhy/State
  //   /NodeList/[i]/DeviceList/[i]/$ns3::WifiNetDevice/Mac/MacTxDrop
  ///NodeList/[i]/DeviceList/[i]/$ns3::WifiNetDevice/Phy
}
//xtt add udp drop
void
StatsFlows::UdpPacketDrop (std::string context, Ptr<const Packet> packet)
{
  NS_LOG_FUNCTION (this << context << packet  );

  m_allPacketsStats.udpDropPkts++;
}

//xtt add ipv4 
void
StatsFlows::Ipv4PacketSent (std::string context, Ptr<const Packet> packet, Ptr<Ipv4> ipv4, uint32_t interface)
{
  NS_LOG_FUNCTION (this << context << packet  );

  m_allPacketsStats.ipv4TxPkts++;
}

void
StatsFlows::Ipv4PacketReceive (std::string context, Ptr<const Packet> packet, Ptr<Ipv4> ipv4,uint32_t interface)
{
  NS_LOG_FUNCTION (this << context << packet );

  m_allPacketsStats.ipv4RxPkts++;
}

/*
void
StatsFlows::Ipv4PacketDrop (std::string context, Ptr<const Packet> packet, DROP_NO_ROUTE, Ptr<Ipv4> ipv4,uint32_t interface)
{
  NS_LOG_FUNCTION (this << context << packet );
 
  m_allPacketsStats.ipv4DropPkts++;
}
*/
//xtt add mac packet send and receive
void
StatsFlows::MacPacketSent (std::string context, Ptr<const Packet> packet)
{
  NS_LOG_FUNCTION (this << context << packet  );
  if(packet->GetSize () > 200)
  {
  m_allPacketsStats.macTxPkts++;
  }
}

void
StatsFlows::MacPacketReceive (std::string context, Ptr<const Packet> packet)
{
  NS_LOG_FUNCTION (this << context << packet );
  if(packet->GetSize () > 200)
  {
  m_allPacketsStats.macRxPkts++;
  }
}
void
...
//xtt phypacketreceiveOK
void
StatsFlows::PhyPacketReceive (std::string context, Ptr<const Packet> packet, double snr,WifiMode mode, WifiPreamble preamble)
{
  NS_LOG_FUNCTION (this << context << packet << "PHYTX mode=" << mode );
  if(packet->GetSize () > 200)
  {
  m_allPacketsStats.phyRxPkts++;
  }
}
...
//PHY tx packet
  ...
  srs.aap.phyRxPkts = m_allPacketsStats.phyRxPkts;
  srs.aap.macTxPkts = m_allPacketsStats.macTxPkts;
  srs.aap.macRxPkts = m_allPacketsStats.macRxPkts;
  srs.aap.ipv4TxPkts = m_allPacketsStats.ipv4TxPkts;
  srs.aap.ipv4RxPkts = m_allPacketsStats.ipv4RxPkts;
  srs.aap.udpDropPkts = m_allPacketsStats.udpDropPkts;
  ...
  if (IsScalarFileWriteEnabled ())
    {
      ...
      out << std::endl;
      out << "AVERAGE RESULTS, Average of all flows (" << srs.numberOfFlows << "), Summary of all packets" << std::endl;
...
//xtt add
      ...
      out << "PHY Rx packets:,"<< "," << srs.aap.phyRxPkts << std::endl;
      out << "MAC Tx packets:,"<< "," << srs.aap.macTxPkts << std::endl;
      out << "MAC Rx packets:,"<< "," << srs.aap.macRxPkts << std::endl;
      out << "IPV4 Tx packets:,"<< "," << srs.aap.ipv4TxPkts << std::endl;
      out << "IPV4 Rx packets:,"<< "," << srs.aap.ipv4RxPkts << std::endl;
      out << "UDP Drop packets:,"<< "," << srs.aap.udpDropPkts << std::endl;
