...
struct ScalarData
{
  ScalarData (double histRes = 0.0001) 
    : totalRxPackets (0),
	    ...
	    udpDropPkts (0),
	    phyTxBytes (0),
	    delayHist (histRes)  // 0.1 ms, default xtt add
  {
    delayHist.Clear ();
  }
...
///xtt add
  ...
  uint16_t ipv4TxPkts;
  uint16_t ipv4RxPkts;
  uint16_t ipv4DropPkts;
  uint16_t udpDropPkts;
...
//xtt revise
 // void PhyPacketSent (Ptr<const Packet> packet);
...
//xtt add
  void PhyPacketReceive (std::string context, Ptr<const Packet> packet, double snr,WifiMode mode, WifiPreamble preamble);
  void MacPacketReceive (std::string context, Ptr<const Packet> packet);
  void MacPacketSent (std::string context, Ptr<const Packet> packet);
  void Ipv4PacketSent (std::string context, Ptr<const Packet> packet, Ptr<Ipv4> ipv4, uint32_t interface);
  void Ipv4PacketReceive (std::string context, Ptr<const Packet> packet, Ptr<Ipv4> ipv4,uint32_t interface);
  //void Ipv4PacketDrop (std::string context, Ptr<const Packet> packet, DROP_NO_ROUTE, Ptr<Ipv4> ipv4,uint32_t interface);
  void UdpPacketDrop (std::string context, Ptr<const Packet> packet);
  RunSummary Finalize ();
