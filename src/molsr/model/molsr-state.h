...
/// \ingroup molsr
/// This class encapsulates all data structures needed for maintaining internal state of an molsr node.
class MolsrState
{
  //  friend class Olsr;
...
protected:
  ...
  McRouterSet m_mcrouterset;//table contains all the nodes with multicast capabilities
  McTreeSet m_mctreeset;//the information dealing with different multicast group that this  node is participating in 
  MulticastGroupSet m_multicastGroupSet; //
public:
  MolsrState()
  {
  }
  ...
  //初始化组播组
  //获得组播地址
  const MulticastGroupSet &GetGroupAddrSet() const
  {
    return m_multicastGroupSet;
  }
  /**
   * 查询组播组
   */
  GroupAssocTuple *FindGroupAssocTuple(const Ipv4Address &groupAddr);
  /**
   * 退出组播组
   */
  void EraseGroupAssocTuple(const GroupAssocTuple &tuple);

  ///加入组播组
  void InsertGroupAssocTuple(const GroupAssocTuple &tuple);
  ...
  /**
     * Gets the m_mcrouterset set.
     * \returns The topology set.
     * xtt-----------molsr
     */
  const McRouterSet &GetMcRouterSet() const
  {
    return m_mcrouterset;
  }
  /**
     * Finds a topology tuple.
     * \param destAddr The destination address.
     ...
     * \returns The topology tuple, or a null pointer if no match.
     */
  McRouterTuple *FindMcRouterTuple(const Ipv4Address &MmAddr);
  /**
     * Erases a topology tuple.
     * \param tuple The tuple to erase.
     */
  void EraseMcRouterTuple(const McRouterTuple &tuple);
  /**
     * Inserts a topology tuple.
     * \param tuple The tuple to insert.
     */
  void InsertMcRouterTuple(const McRouterTuple &tuple);

  //mc tree table set

  const McTreeSet &GetMcTreeSet() const
  {
    return m_mctreeset;
  }

  McTreeTuple *FindMcTreeTuple(const Ipv4Address &mtsourceAddr,
  ...
  /**
       ...
       * \param tuple The tuple to erase.
       */
  void EraseMcTreeTuple(const McTreeTuple &tuple);
  /*
       * Inserts a topology tuple.
       * \param tuple The tuple to insert.
       */
  void InsertMcTreeTuple(const McTreeTuple &tuple);
};
}
} // namespace olsr,ns3
