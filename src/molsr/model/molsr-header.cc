...
#define MOLSR_MSG_HEADER_SIZE 12
#define MOLSR_PKT_HEADER_SIZE 4
...
{
  uint32_t size = MOLSR_MSG_HEADER_SIZE;
  ...
  ///xtt--------molsr
  case MC_CLAIM:
    size += m_message.mclaim.GetSerializedSize();
    break;
  case SOURCE_CLAIM:
    size += m_message.sourceclaim.GetSerializedSize();
    break;
  case CONFIRM_PARENT:
    size += m_message.confirmparent.GetSerializedSize();
    break;
  case LEAVE:
    size += m_message.leave.GetSerializedSize();
    break;
  default:
    NS_ASSERT(false);
  }
...
void MessageHeader::Serialize(Buffer::Iterator start) const
{
  ...
  //xtt------molsr
  case MC_CLAIM:
    m_message.mclaim.Serialize(i);
    break;
  case SOURCE_CLAIM:
    m_message.sourceclaim.Serialize(i);
    break;
  case CONFIRM_PARENT:
    m_message.confirmparent.Serialize(i);
    break;
  case LEAVE:
    m_message.leave.Serialize(i);
    break;
  default:
    NS_ASSERT(false);
  }
...
MessageHeader::Deserialize(Buffer::Iterator start)
{
  ...
  NS_ASSERT(m_messageType >= HELLO_MESSAGE && m_messageType <= LEAVE);
  m_vTime = i.ReadU8();
  ...
  size = MOLSR_MSG_HEADER_SIZE;
  switch (m_messageType)
  {
  case MID_MESSAGE:
    size += m_message.mid.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case HELLO_MESSAGE:
    size += m_message.hello.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case TC_MESSAGE:
    size += m_message.tc.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case HNA_MESSAGE:
    size += m_message.hna.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  // xtt---molsr
  case MC_CLAIM:
    size += m_message.mclaim.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case SOURCE_CLAIM:
    size += m_message.sourceclaim.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case CONFIRM_PARENT:
    size += m_message.confirmparent.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  case LEAVE:
    size += m_message.leave.Deserialize(i, m_messageSize - MOLSR_MSG_HEADER_SIZE);
    break;
  default:
    NS_ASSERT(false);
  }
...
//xtt--------------molsr
// ---------------- OLSR MC_CLAIM Message -------------------------------

uint32_t
MessageHeader::Mclaim::GetSerializedSize(void) const
{
  return 4 + this->neighborAddresses.size() * IPV4_ADDRESS_SIZE;
}

void MessageHeader::Mclaim::Print(std::ostream &os) const
{
  /// \todo
}

void MessageHeader::Mclaim::Serialize(Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
...
uint32_t
MessageHeader::Mclaim::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  Buffer::Iterator i = start;
...
// ---------------- OLSR SOURCE_CLAIM Message -------------------------------

uint32_t
MessageHeader::Sourceclaim::GetSerializedSize(void) const
{
  return 4 + 4 + this->neighborAddresses.size() * IPV4_ADDRESS_SIZE;
}

void MessageHeader::Sourceclaim::Print(std::ostream &os) const
{
  /// \todo
}

void MessageHeader::Sourceclaim::Serialize(Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
  i.WriteHtonU16(this->ansn);
  i.WriteHtonU16(0); // Reserved
                     //组播组地址32bits
  i.WriteHtonU32(this->multicastGroupAddress.Get());
...
uint32_t
MessageHeader::Sourceclaim::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  Buffer::Iterator i = start;
  NS_ASSERT(messageSize >= 4);
  ...
  this->ansn = i.ReadNtohU16();
  i.ReadNtohU16(); // Reserved
  this->multicastGroupAddress = Ipv4Address(i.ReadNtohU32());
  NS_ASSERT((messageSize - 8) % IPV4_ADDRESS_SIZE == 0);
  //组播组地址和邻居地址不一样，假设一个组播组有一个邻居选择节点
  int numAddresses = (messageSize - 8) / IPV4_ADDRESS_SIZE / 2;
...
// ---------------- OLSR CONFIRM_PARENT Message -------------------------------

uint32_t
MessageHeader::Confirmparent::GetSerializedSize(void) const
{
  return 3 * this->group.size() * IPV4_ADDRESS_SIZE; //头部的长度
}

void MessageHeader::Confirmparent::Print(std::ostream &os) const
{
  /// \todo
}

void MessageHeader::Confirmparent::Serialize(Buffer::Iterator start) const
{
  Buffer::Iterator i = start;

  for (size_t n = 0; n < this->group.size(); ++n)
  {
    i.WriteHtonU32(this->group[n].parentAddresses.Get());
    i.WriteHtonU32(this->group[n].multicastGroupAddress.Get());
    i.WriteHtonU32(this->group[n].multicastSourceAddress.Get());
  }
}

uint32_t
MessageHeader::Confirmparent::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  Buffer::Iterator i = start;

  NS_ASSERT(messageSize % (IPV4_ADDRESS_SIZE * 3) == 0);
  int numAddresses = messageSize / IPV4_ADDRESS_SIZE / 3;
  this->group.clear();
  for (int n = 0; n < numAddresses; ++n)
  {
    Ipv4Address parentAddresses(i.ReadNtohU32());
    Ipv4Address multicastGroupAddress(i.ReadNtohU32());
    Ipv4Address multicastSourceAddress(i.ReadNtohU32());
    this->group.push_back((Group){parentAddresses, multicastGroupAddress, multicastSourceAddress});
  }
  return messageSize;
}
...
uint32_t
MessageHeader::Leave::GetSerializedSize(void) const
{
  return 3 * this->group.size() * IPV4_ADDRESS_SIZE; //头部的长度
}

void MessageHeader::Leave::Print(std::ostream &os) const
{
  /// \todo
}

void MessageHeader::Leave::Serialize(Buffer::Iterator start) const
{
  Buffer::Iterator i = start;
...
uint32_t
MessageHeader::Leave::Deserialize(Buffer::Iterator start, uint32_t messageSize)
{
  Buffer::Iterator i = start;
