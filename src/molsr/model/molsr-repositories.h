...
struct GroupAssocTuple
{
  /// group address
  Ipv4Address groupAddr;
  //node address
  std::set<Ipv4Address> NodeList;
  /// Time at which this tuple expires and must be removed.
  Time time;
};

static inline bool
operator == (const GroupAssocTuple &a, const GroupAssocTuple &b)
{
  return (a.groupAddr == b.groupAddr);
}

static inline std::ostream&
operator << (std::ostream &os, const GroupAssocTuple &tuple)
{
  os << "GroupAssocTuple(groupAddr=" << tuple.groupAddr
     << ", time=" << tuple.time << ")";
  return os;
}
...
static inline bool
operator == (const McRouterTuple &a, const McRouterTuple &b)
{
  return (a.MmAddr == b.MmAddr);
}

static inline std::ostream&
operator << (std::ostream &os, const McRouterTuple &tuple)
{
  os << "McRouterTuple(MmAddr=" << tuple.MmAddr
     << ", expirationTime=" << tuple.expirationTime
     << ")";
  return os;
}
...
struct McTreeTuple
{
  ...
  Ipv4Address mtsourceAddr;
  uint16_t sequenceNumber;
  Time mtsourceTime;
  Ipv4Address mtgroupAddr;
  //父母节点是唯一的
  Ipv4Address mtparentAddr;
  Time mtchildTime;
  //孩子集合
  std::set<Ipv4Address> mtchildList;
  //std::vector<Ipv4Address> mtchildList;
  /// Time at which this tuple expires and must be removed.
};

static inline bool
operator == (const McTreeTuple &a, const McTreeTuple &b)
{
  return (a.mtsourceAddr == b.mtsourceAddr
          && a.mtgroupAddr == b.mtgroupAddr
          && a.mtparentAddr == b.mtparentAddr);
}

static inline std::ostream&
operator << (std::ostream &os, const McTreeTuple &tuple)
{
  os << "McTreeTuple(mtsourceAddr=" << tuple.mtsourceAddr
     << ", mtgroupAddr=" << tuple.mtgroupAddr
     << ", mtparentAddr=" << tuple.mtparentAddr
     << ", mtsourceTime=" << tuple.mtsourceTime
     << ", mtchildTime=" << tuple.mtchildTime
     << ")";
  return os;
}
...
typedef std::vector<GroupAssocTuple>            MulticastGroupSet;
typedef std::vector<McRouterTuple>          McRouterSet; //!< mc router Set type.
typedef std::vector<McTreeTuple>           McTreeSet; //!< mc tree Set type.


}
}  // namespace ns3, olsr

#endif /* MOLSR_REPOSITORIES_H */
