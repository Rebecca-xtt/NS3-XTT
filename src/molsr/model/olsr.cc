...
{
  static TypeId tid = TypeId ("ns3::molsr::RoutingProtocol")
   ...
   .AddTraceSource ("NeighborSetChanged", "The OLSR routing table has changed causing the neighbor set to change.",
                    MakeTraceSourceAccessor (&RoutingProtocol::m_neighborSetChanged),
                     "ns3::molsr::RoutingProtocol::NeighborSetChangeTracedCallback")
  ;
  return tid;
}
  ...
  for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
    {
      ...
      NS_LOG_DEBUG ("Set callback to RecvOlsr");
...
//修改，，，，，两跳多播节点被一#跳多播节点覆盖，具有多播能力的节点优先选择？？在哪里改？
...
void RoutingProtocol::MprComputation()
{
  ...
  {
    //判断一跳邻居是不是多播节点
    const McRouterTuple *mcRouter_tuple1 = m_state.FindMcRouterTuple(neighbor->neighborMainAddr);

    if (neighbor->status == NeighborTuple::STATUS_SYM && mcRouter_tuple1 != NULL) 
    {
      N.push_back(*neighbor);//一跳多播对称邻居
    }
  }
  ...
  {
    //多播节点
    const McRouterTuple *mcRouter_tuple2 = m_state.FindMcRouterTuple(twoHopNeigh->twoHopNeighborAddr);
    // 该两跳邻居不是一跳邻居且不是自身
    ///跳过非多播节点和自身是两跳的节点
    if (twoHopNeigh->twoHopNeighborAddr == m_mainAddress || mcRouter_tuple2 == NULL)
    {
      continue;
    }
  ...
  for (TwoHopNeighborSet::const_iterator twoHopNeigh = N2.begin(); twoHopNeigh != N2.end(); twoHopNeigh++)
  {
    ...
    {
      if (otherTwoHopNeigh->twoHopNeighborAddr == twoHopNeigh->twoHopNeighborAddr && otherTwoHopNeigh->neighborMainAddr != twoHopNeigh->neighborMainAddr)
      {
        onlyOne = false;
        break;
      }
       ...
       twoHopNeigh != N2.end();)
  {
    if (coveredTwoHopNeighbors.find(twoHopNeigh->twoHopNeighborAddr) != coveredTwoHopNeighbors.end())
    {
      ...
      twoHopNeigh = N2.erase(twoHopNeigh); //返回的是位置
    }
    else
    {
      twoHopNeigh++;
  ...
  while (N2.begin() != N2.end())
  {
    ...
    if (max != NULL)
    {
      ...
      NS_LOG_LOGIC(N2.size() << " 2-hop neighbors left to cover!"); //剩余未被覆盖的节点
    }
  }
...
void
RoutingProtocol::ProcessTc (const molsr::MessageHeader &msg,
                            const Ipv4Address &senderIface)
{
...
void
RoutingProtocol::ProcessMid (const molsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
...
void
RoutingProtocol::ProcessHna (const molsr::MessageHeader &msg,
                             const Ipv4Address &senderIface)
{
  ...
  for (std::vector<molsr::MessageHeader::Hna::Association>::const_iterator it = hna.associations.begin ();
...
RoutingProtocol::SendHello ()
{
  ...
  std::vector<molsr::MessageHeader::Hello::LinkMessage>
...
RoutingProtocol::EnergyCalculator(void)
{
	...
	if(m_ipv4->GetObject<Node>()->GetId() == 3)
	{
		...
		std::ofstream energyOverhead("energy_overhead_D.txt", std::ofstream::out | std::ofstream::app);//以输出或者追加的方式打开
		if (energyOverhead.is_open())
		{
			energyOverhead << "开销："<<overHead << "\n"<< "丢包数" << dropCount << "\n" ;
                       //每发一次包就累加，最后一个是总开销
		}
		else
		{
