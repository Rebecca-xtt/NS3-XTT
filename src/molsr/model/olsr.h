...
public:
 ...
 /**
  ...
  /**
   *关联IPV4 staticrouting  table 到olsr routing protocol.
   */
...
private:
  ...
  bool FindSendEntry(const RoutingTableEntry &entry,
                     RoutingTableEntry &outEntry) const;
  // From Ipv4RoutingProtocol  两个虚函数用于路由包和转发，output用于本地发出的数据包，input用于转发或传输接收到的包。
 ...
 virtual void PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit = Time::S) const ;
   ...
   *
   */
  void ProcessTc(const molsr::MessageHeader &msg,
                 const Ipv4Address &senderIface);
   ...
   * \param senderIface the address of the interface where the message was sent from.
   */
  void ProcessMid(const molsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
   ...
   *
   */
  void ProcessHna(const molsr::MessageHeader &msg,
                  const Ipv4Address &senderIface);
